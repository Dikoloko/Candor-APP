      ...
      <div className="md:grid md:grid-cols-[1fr_300px] md:gap-8 md:items-start">
        ...
        <div>
          ...
          <motion.div
            ...
            className="mb-6"
          >
            <p className="text-white font-bold text-xl md:text-2xl mb-1">Competitor Quiz</p>
          ...
          >
            {cards.map(({ to, icon: Icon, title, desc, color, border, iconColor, emoji }) => (
              <motion.button
                ...
                variants={item}
                onClick={() => navigate(to)}
                className={`card bg-gradient-to-br ${color} border ${border} p-4 md:p-5 text-left active:scale-95 transition-transform duration-100`}
              >
                <div className="text-2xl mb-2">{emoji}</div>
                <div className={`mb-1 ${iconColor}`}>
                  <Icon size={20} strokeWidth={2} />
                </div>
                <div className="font-semibold text-white text-sm md:text-base mb-0.5">{title}</div>
                <div className="text-white/50 text-xs md:text-sm leading-tight">{desc}</div>
              </motion.button>
            ))}
          </motion.div>
        </div>
        ...
        <div className="mt-6 md:mt-0 space-y-4">
          ...
          >
            <div className="card p-3 md:p-4 text-center md:flex md:items-center md:text-left md:gap-3">
              <BookOpen size={18} className="text-candor-teal mx-auto md:mx-0 mb-1 md:mb-0 shrink-0" />
              <div>
                <div className="num text-lg font-bold text-white">{todayCount}</div>
                <div className="text-[10px] md:text-xs text-white/50">Vragen vandaag</div>
              </div>
            </div>
            <div className="card p-3 md:p-4 text-center md:flex md:items-center md:text-left md:gap-3">
              <TrendingUp size={18} className="text-candor-green mx-auto md:mx-0 mb-1 md:mb-0 shrink-0" />
              <div>
                <div className="num text-lg font-bold text-white">{stats?.gemScore ?? '—'}%</div>
                <div className="text-[10px] md:text-xs text-white/50">Gem. score</div>
              </div>
            </div>
            <div className="card p-3 md:p-4 text-center md:flex md:items-center md:text-left md:gap-3">
              <Award size={18} className="text-candor-orange mx-auto md:mx-0 mb-1 md:mb-0 shrink-0" />
              <div>
                <div className="num text-lg font-bold text-white">{stats?.besteScore ?? '—'}%</div>
                <div className="text-[10px] md:text-xs text-white/50">Beste score</div>
              </div>
            </div>
          </motion.div>
